...
public class BluetoothService {
    ...
    public class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            byte[] storageBuffer = new byte[1024];
            ...
            while (true) {
                try {
                    ...
                    for(int idx = 0; idx < bytes; ++idx) {
                        byte ch = readBuffer[idx];
                        if(ch == ';') {
                            String msg = new String(storageBuffer, 0, storageBufferIndex);
                            String[] parts = msg.split(",");
                            myHandler.obtainMessage(Constants.MESSAGE_READ, bytes, -1, parts).sendToTarget();
                            storageBufferIndex = 0;
                        }
                        else {
                            storageBuffer[storageBufferIndex++] = ch;
                        }
                    }
                } catch (IOException e) {
